...
namespace Mvp.Xml.Common.XPath
{
	...
	public static class XPathCache
	{
		...
		/// </summary>
		private static IDictionary<string, XPathExpression> Cache { get; } = new Dictionary<string, XPathExpression>();
		...
		{
		    if (!Cache.TryGetValue(expression, out XPathExpression expr))
			{
